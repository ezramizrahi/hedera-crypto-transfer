...
class Transaction extends _Executable.default {
  ...
  schedule() {
    ...
    if (this._signedTransactions.length != 1) {
      throw new Error("`Transaction.schedule()` requires `Transaction` to have a single node `AccountId` set");
    }
    ...
    return SCHEDULE_CREATE_TRANSACTION[0]().setTransaction(this).setNodeAccountIds(this._nodeIds);
  }
  /**
